...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s - UnifiedBot - %(levelname)s - %(message)s"
)
...
class GptWebSearchSource(BaseSource):
    ...
    async def fetch(self, query: str) -> str:
        prompt = (
            f"Produce a factual research brief. Prefix uncertain claims. Under 600 words.\n"
...
def _fetch_yfinance_data_sync(ticker: str) -> str:
    ...
    try:
        ...
        high52 = tk.info.get("fiftyTwoWeekHigh", "N/A")
        low52  = tk.info.get("fiftyTwoWeekLow",  "N/A")
        ...
        return (
            f"--- LIVE MARKET DATA ({ticker}) ---\n"
            f"Spot Price: {spot:.2f} | 52-Week Range: {low52} - {high52}\n"
            f"Annual Volatility: {vol:.2%} | "
            f"1-Mo Random Walk: P10={rw_p10:.2f}, P50={spot:.2f}, P90={rw_p90:.2f}\n"
        )
    except Exception:
        return ""
...
class ForecastValidator:
    ...
    def __init__(self, db_path: str = "unified_bot_validation.db"):
    ...
    def _init_db(self) -> None:
        with sqlite3.connect(self._db_path) as conn:
            conn.execute(
                ...
                "strategy TEXT, prediction_value TEXT, confidence_score REAL, flagged INTEGER, ts REAL)"
            )
            conn.commit()
        ...
        try:
            with sqlite3.connect(self._db_path) as conn:
                rows = conn.execute(
                    ...
                    "SUM(flagged) as n_flagged FROM forecast_ledger "
...
class ResearchCache:
    def __init__(self, db_path: str = "unified_bot_cache.db"):
        self._db_path = db_path
        with sqlite3.connect(self._db_path) as conn:
            conn.execute(
...
def is_meaningful_research_text(txt: str) -> bool:
    if not txt:
        return False
    if "failed:" in txt.lower() or "error:" in txt.lower() or "timeout" in txt.lower():
        return False
...
def derive_numeric_fallback_bounds(
    question: NumericQuestion,
...
def log_forecast_for_calibration(
    question, prediction_value, reasoning, models_used, research_used, searchers_used
):
    entry = {
...
def build_reasoning_block(
    question, forecast_text: str, base_rate_text: str,
    methodology_text: str, strategy: str, profile: QuestionProfile,
    searchers_used: List[str], minibench: bool, ext_factor: float
) -> str:
    minibench_tag = " (aggressive minibench mode)" if minibench else ""
    return clean_indents(f"""
    My forecast: {forecast_text}
    
    I anchored on a base rate of {base_rate_text} and adjusted based on recent evidence.
    The question falls in {profile.domain}; the key uncertainty lies in {profile.geography or 'broader trends'}.
...
async def invoke_with_free_model_fallback(
    ...
    for model_idx, model in enumerate(_FREE_MODEL_CHAIN):
        try:
            ...
            if (result and not result.startswith(label)
                    and "error" not in result.lower()
                    and "timed out" not in result.lower()):
...
class UnifiedForecastBot(ForecastBot):
    """
    Unified forecasting bot combining:

    From HybridPreMortemBot:
      - 3-model committee voting (GPT-5 × 2, Claude × 1) — one vote per forecast
      - NewsAPI research source
      - Median aggregation across committee

    From Yrambot:
      - Question profiling (domain, geo, time horizon)
      - Modelling strategy selection (trend / analogical / market_signal / base_rate)
      - Multi-source research with planning, synthesis, and caching
      - yFinance live market data injection
      - Extremization (standard + aggressive minibench mode)
      - Community-prediction crowd blending
      - SQLite validation ledger + calibration log
      - Selective tournament confidence gate
...
<<<<<<< HEAD
            ...
            llms = {
                "default":       claude_llm,      # Claude Sonnet — primary forecaster
                ...
                "perplexity":    sonar_pro_llm,
                "gpt5_search":   sonar_base_llm,
            }
...
<<<<<<< HEAD
        ...
        # GPT fallback if Tavily unavailable
        if not tavily_src.is_available():
            self._sources.register(GptWebSearchSource(llm=sonar_fallback))

        self._sources.register(PerplexitySonarSource(llm=self.get_llm("perplexity", "llm")))
        self._sources.register(Gpt5SearchSource(llm=self.get_llm("gpt5_search", "llm")))
...
>>>>>>> 8ac85e2bcefa4d87f5eb857342fafe0c02625ccd
    ...
    async def _plan_queries(
        ...
        hint = {
            ...
            "multiple_choice": "relative likelihood",
            "numeric":         "quantitative benchmarks",
        }.get(question_type, "base rates")
        prompt = (
            f"Generate 5 to 7 precise web-search queries for domain: {profile.domain}.\n"
        ...
        return [
            ...
            f"{question.question_text} historical base rate",
        ]
    ...
    async def _multi_source_research_bundle(
        ...
        all_queries = list(dict.fromkeys(
            llm_queries + [f"Metaculus community probability {question.question_text}"]
        ))
    ...
    async def _synthesize_research(
        ...
        prompt = (
            "Synthesize evidence into a 4-part research brief "
            "(Base rate, Updates, Uncertainties, Signposts). Max 2400 chars.\n"
            f"Question: {question.question_text}\nSources:\n{metaculus_block}\n"
            f"[Web Research]\n{source_bundle}"
        )
    ...
    async def run_research(self, question: MetaculusQuestion) -> str:
        async with self._concurrency_limiter:
            ...
            q_type   = ("numeric"         if isinstance(question, NumericQuestion)        else
                        "multiple_choice" if isinstance(question, MultipleChoiceQuestion) else
                        "binary")
            ...
            fin_data   = ""
            is_finance = ("market-pulse" in (self._active_tournament or "")
                          or profile.domain in ("finance", "economics"))
            if is_finance and YFINANCE_AVAILABLE:
                try:
                    ticker_prompt = (
                        "Extract the Yahoo Finance ticker for this question. "
                        "Reply ONLY with the ticker or NONE.\n"
                        f"Question: {question.question_text}"
                    )
...
>>>>>>> 8ac85e2bcefa4d87f5eb857342fafe0c02625ccd
            ...
            final = (
                ...
                f"\n\n[Raw Web Research]\n{source_bundle}"
                if source_bundle
                else f"{fin_data}{metaculus_block}\n\n[Research Summary]\n{synthesized}"
            )
            ...
            searchers_used = (
                ["metaculus"] + [s.name for s in self._sources.available_sources()]
                if source_bundle else ["metaculus"]
            )
    ...
    def _selective_confidence_gate(
        self,
        qid: str,
        p_final: Any,
        profile: QuestionProfile,
        ...
        if str(self._active_tournament) in SELECTIVE_TOURNAMENTS:
            ...
            if confidence < self._validator.LOW_CONFIDENCE_THRESHOLD:
                raise RuntimeError(
                    f"Selective Forecasting Gate: Confidence {confidence:.2f} too low "
                    f"for Q{qid}. Skipping."
                )

    # ── Committee (pre-mortem) single-model forecast ──────────────────────

    async def _committee_single_forecast(
        self,
        question: MetaculusQuestion,
        research: str,
        profile: QuestionProfile,
        ...
        """
        Run one committee member's forecast. Returns (prediction, reasoning_str).
        Model string is a raw OpenRouter path, not a llms-dict key.
        """
        llm = GeneralLlm(model=model_string, temperature=0.15, timeout=90, allowed_tries=2)
        ...
        if isinstance(question, BinaryQuestion):
            ...
            Write your analysis, then end with exactly: "Probability: ZZ%"
            """)
            raw  = await with_timeout(llm.invoke(prompt), LLM_TIMEOUT_S, "committee_binary")
            pred: BinaryPrediction = await structure_output(
                raw, BinaryPrediction,
            ...
            """)
            raw    = await with_timeout(llm.invoke(prompt), LLM_TIMEOUT_S, "committee_mc")
            result = await structure_output(
                raw, PredictedOptionList,
                model=self.get_llm("parser", "llm"),
                additional_instructions=f"Names must match: {question.options}"
            )
            ...
            normed = PredictedOptionList(predicted_options=[
                PredictedOption(option_name=opt, probability=float(probs[opt] / total))
                for opt in question.options
            ])
            return normed, str(raw)
            ...
            Write analysis, then provide a JSON array of percentiles:
            [{{"percentile":0.1,"value":<num>}}, {{"percentile":0.2,"value":<num>}},
             {{"percentile":0.4,"value":<num>}}, {{"percentile":0.6,"value":<num>}},
             {{"percentile":0.8,"value":<num>}}, {{"percentile":0.9,"value":<num>}}]
            """)
            raw = await with_timeout(llm.invoke(prompt), LLM_TIMEOUT_S, "committee_numeric")
            try:
                ...
                repair_prompt = (
                    f"Convert to valid JSON array of Percentile objects.\n"
                    f"{sanitize_numeric_json(str(raw))}"
                )
                repaired = await invoke_with_free_model_fallback(
                    repair_prompt, temperature=0.15, timeout_s=LLM_TIMEOUT_S,
                    label="committee_num_repair"
                )
    ...
    # ── Core ensemble helpers (Yrambot-style dual-model) ─────────────────

    async def _forecast_binary_core(
        self,
        ...
        base_str = (f"Community prob: {base:.4f}"
                    if isinstance(base, (int, float)) else "No community probability.")
        prompt = (
            f"You are an expert superforecaster. Produce a calibrated probability.\n"
        ...
        raw  = await with_timeout(
            self.get_llm(model_key, "llm").invoke(prompt), LLM_TIMEOUT_S, f"binary_llm_{model_key}"
        )
        pred = await structure_output(raw, BinaryPrediction, model=self.get_llm("parser", "llm"))
        return clamp01(float(pred.prediction_in_decimal)), str(raw)

    async def _forecast_mc_core(
        self,
        ...
        options_list = "\n".join(f"  - {opt}" for opt in question.options)
        prompt = (
            f"You are an expert superforecaster. Assign probabilities summing to 1.0.\n"
            f"{ModellingStrategy.get_prompt_block(strategy, profile)}\n"
            f"Question: {question.question_text}\nOptions:\n{options_list}\n"
            f"Research summary:\n{research}\n"
            'Return JSON ONLY: {"predicted_options": [{"option_name": "<exact name>", "probability": <decimal>}, ...]}'
        ...
        raw    = await with_timeout(
            self.get_llm(model_key, "llm").invoke(prompt), LLM_TIMEOUT_S, f"mc_llm_{model_key}"
        )
        result = await structure_output(
            raw, PredictedOptionList,
            model=self.get_llm("parser", "llm"),
    ...
    async def _forecast_numeric_core(
        self,
        question: NumericQuestion,
        research: str,
        profile: QuestionProfile,
        ...
        prompt = (
            f"Produce a calibrated probability distribution over possible outcomes.\n"
            f"{ModellingStrategy.get_prompt_block(strategy, profile)}\n"
            f"Question: {question.question_text}\n"
            f"Lower bound: {getattr(question, 'lower_bound', 'N/A')}, "
            f"Upper bound: {getattr(question, 'upper_bound', 'N/A')}\n"
            f"Research summary:\n{research}\n"
            'Return JSON array ONLY: [{"percentile":0.1,"value":<num>}, {"percentile":0.2,"value":<num>}, '
            '{"percentile":0.4,"value":<num>}, {"percentile":0.6,"value":<num>}, '
            '{"percentile":0.8,"value":<num>}, {"percentile":0.9,"value":<num>}]'
        )
        raw = await with_timeout(
            self.get_llm(model_key, "llm").invoke(prompt), LLM_TIMEOUT_S, f"num_llm_{model_key}"
        )
        try:
            percentile_list = await structure_output(
                raw, list[Percentile], model=self.get_llm("parser", "llm")
            ...
            try:
                ...
                raise
        return enforce_numeric_constraints(
            interpolate_missing_percentiles(percentile_list, [0.1, 0.2, 0.4, 0.6, 0.8, 0.9]),
            question,
        ), str(raw)
    ...
    async def _run_forecast_on_binary(
        ...
        # ── Stage 1: Yrambot dual-model ensemble ─────────────────────────
        raw_ps: List[float] = []
        for model_key in ("default", "default_alt"):
            try:
                p, _ = await self._forecast_binary_core(
                    question, research, profile, strategy, model_key=model_key
                )
                raw_ps.append(p)
            except Exception as e:
                logger.warning(f"[UnifiedBot] binary ensemble failed ({model_key}): {e}")

        # ── Stage 2: Committee (pre-mortem) third vote ────────────────────
        # Third committee slot always uses Claude (index 2 of _COMMITTEE_MODELS)
        try:
            committee_p, _ = await self._committee_single_forecast(
                question, research, profile, strategy, _COMMITTEE_MODELS[2]
            )
            raw_ps.append(committee_p)
        except Exception as e:
            logger.warning(f"[UnifiedBot] committee vote failed: {e}")

        raw_p = statistics.median(raw_ps) if raw_ps else clamp01(
            float(base) if isinstance(base, (int, float)) else 0.5
        )
        ...
        if not self._is_minibench() and isinstance(base, (int, float)):
            ...
            blend_note = (
                "Crowd blending disabled (Minibench Anti-Average Engine active)."
                if self._is_minibench() else "No community anchor available."
            )
        ...
        methodology = f"Superforecasting protocol with {strategy} framework. Committee size: {len(raw_ps)}.\n{blend_note}"
        reasoning   = build_reasoning_block(
            question, f"{p_final:.1%}",
            f"{base:.1%}" if isinstance(base, (int, float)) else "None",
            methodology, strategy, profile,
            self._research_meta.get(qid, {}).get("searchers_used", []),
            self._is_minibench(), self._ext().factor,
        )
        log_forecast_for_calibration(question, p_final, reasoning, ["Ensemble+Committee"], True,
                                     self._research_meta.get(qid, {}).get("searchers_used", []))
    ...
    async def _run_forecast_on_multiple_choice(
        ...
        results: List[PredictedOptionList] = []

        # Dual-model ensemble
        for model_key in ("default", "default_alt"):
            try:
                out, _ = await self._forecast_mc_core(
                    question, research, profile, strategy, model_key=model_key
                )
                results.append(out)
            except Exception as e:
                logger.warning(f"[UnifiedBot] MC ensemble failed ({model_key}): {e}")

        # Committee vote (Claude)
        try:
            committee_out, _ = await self._committee_single_forecast(
                question, research, profile, strategy, _COMMITTEE_MODELS[2]
            )
            if isinstance(committee_out, PredictedOptionList):
                results.append(committee_out)
        except Exception as e:
            logger.warning(f"[UnifiedBot] committee MC vote failed: {e}")

        if results:
            merged_probs: Dict[str, float] = {}
            for opt in question.options:
                values = [
                    next((o.probability for o in r.predicted_options if o.option_name == opt), 0.0)
                    for r in results
                ]
                merged_probs[opt] = statistics.median(values)
            total = sum(merged_probs.values()) or 1.0
            out   = PredictedOptionList(predicted_options=[
                PredictedOption(option_name=opt, probability=merged_probs[opt] / total)
                for opt in question.options
            ])
        else:
            out = PredictedOptionList(predicted_options=[
                PredictedOption(option_name=opt, probability=1.0 / len(question.options))
                for opt in question.options
            ])

        if self._is_minibench():
            extremized = {
                o.option_name: extremize_probability(o.probability, self._ext_cfg_minibench)
                for o in out.predicted_options
            }
        ...
        self._validator.validate(question, profile, strategy,
                                 [o.probability for o in out.predicted_options], research)

        methodology = (
            f"Superforecasting protocol with {strategy} framework. "
            f"Committee size: {len(results)}.\n"
            + ("Per-option logit extremization applied." if self._is_minibench() else "Standard distribution.")
        )
        reasoning = build_reasoning_block(
            question,
            ", ".join([f"{x.option_name}: {x.probability:.1%}" for x in out.predicted_options]),
            "Qualitative anchor", methodology, strategy, profile,
            self._research_meta.get(qid, {}).get("searchers_used", []),
            self._is_minibench(), self._ext().factor,
        )
        time.sleep(PUBLISH_SLEEP_S)
    ...
    async def _run_forecast_on_numeric(
        ...
        all_percentiles: List[List[Percentile]] = []

        for model_key in ("default", "default_alt"):
            try:
                validated, _ = await self._forecast_numeric_core(
                    question, research, profile, strategy, model_key=model_key
                )
                all_percentiles.append(validated)
            except Exception as e:
                logger.warning(f"[UnifiedBot] numeric ensemble failed ({model_key}): {e}")
        ...
        try:
            ...
            if isinstance(committee_out, list):
                all_percentiles.append(committee_out)
        except Exception as e:
            logger.warning(f"[UnifiedBot] committee numeric vote failed: {e}")
        ...
        if all_percentiles:
            ...
            for p in target_ps:
                values = [
                    next((x.value for x in result if x.percentile == p), 0.0)
                    for result in all_percentiles
                ]
            ...
            validated = enforce_numeric_constraints(
                [Percentile(percentile=p, value=v)
                 for p, v in zip([0.1, 0.2, 0.4, 0.6, 0.8, 0.9], vals)],
                question,
            )

        lower = (getattr(question, "lower_bound", None)
                 or getattr(question, "nominal_lower_bound", None))
        ...
        methodology = (
            f"Superforecasting protocol with {strategy} framework. "
            f"Committee size: {len(all_percentiles)}.\n"
            + ("Market-Pulse Tail Fattening applied to P10/P90."
               if "market-pulse" in (self._active_tournament or "")
    ...
    async def forecast_on_tournament(self, tournament_id, *args, **kwargs):
        ...
        if self._is_minibench():
            logger.info(
                f"[UnifiedBot] Minibench detected — "
            ...
            for attempt in range(RETRY_MAX):
                try:
                    ...
                    if any(x in str(e).lower()
                           for x in ("too many requests", "cloudflare", "1015", "429")):
                        logger.error(
