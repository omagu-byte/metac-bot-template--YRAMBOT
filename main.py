...
def extremize(p: float, factor: float = 5.8) -> float:
    ...
    # Add a tiny jitter to avoid exactly 0.5
    if p == 0.5:
...
class Yrambot(ForecastBot):
    ...
    async def _run_forecast_on_binary(self, question: BinaryQuestion, research: str) -> ReasonedPrediction[float]:
        ...
        avg_p = (p1 + p2) / 2
        deviation = abs(p1 - p2)
        # 2% agreement threshold for maximum factor
    ...
    async def _run_forecast_on_multiple_choice(self, question: MultipleChoiceQuestion, research: str) -> ReasonedPrediction[PredictedOptionList]:
        ...
        return ReasonedPrediction(prediction_value=PredictedOptionList(predicted_options=final_options), reasoning=f"### [BOLD MC]\n{research}")
